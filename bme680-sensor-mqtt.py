...
mqtt_base_topic = "/sensors/skjulet"
...
while True:
    ...
    mqtt_client.publish(topic=f"{mqtt_base_topic}/temperature_c", payload=str(temperature))
    mqtt_client.publish(topic=f"{mqtt_base_topic}/pressure_pa", payload=str(pressure))
    mqtt_client.publish(topic=f"{mqtt_base_topic}/humidity_percent", payload=str(humidity))
    mqtt_client.publish(topic=f"{mqtt_base_topic}/gas_ohms", payload=str(gas))
